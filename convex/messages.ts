...
const populateThread = async (ctx: QueryCtx, id: Id<'messages'>) => {
...
const populateReactions = (ctx: QueryCtx, id: Id<'messages'>) => {
  return ctx.db
    .query('reactions')
...
const populateMember = (ctx: QueryCtx, id: Id<'members'>) => {
  return ctx.db.get(id)
}
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Only possible if we are replying in a thread in an 1:1 conversation
    if (!args.conversationId && !args.channelId && args.parentMessageId) {
      const parentMessage = await ctx.db.get(args.parentMessageId)
      if (!parentMessage) {
